...
rooms_base = ["UDR", "Great Room", "FJ", "Stewart", "Library"]
...
class EventCreateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for room_s in rooms_base:
            field_name = "max capacity " + room_s
            self.fields[field_name] = forms.IntegerField(initial=15)
    ...
    def save(self, commit = True, *args, **kwargs):
        ...
        for key,val in self.cleaned_data:
            if "max capacity" in key and val > 0:
                rooms.append(key.replace("max capacity").strip())
